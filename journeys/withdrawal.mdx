...
<Steps>
  <Step title="Register a beneficiary (if needed)">
    ...
    ```bash
    curl --request POST \
      --url {{sandboxUrl}}/api/v1/beneficiaries \
    ...
    Wait for the [`beneficiary.approved`](/webhooks/events) webhook event before proceeding. The beneficiary may be rejected during compliance review.
  </Step>
  ...
  <Step title="Track the withdrawal">
    The withdrawal may go through compliance review. Listen for `operation.created`, `operation.completed`, and `operation.failed` [webhook events](/webhooks/events).
  </Step>
</Steps>
...
- [Webhooks](/webhooks/overview) — monitor all operation events.
