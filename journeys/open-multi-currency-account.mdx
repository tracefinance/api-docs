...
<Steps>
  ...
  <Step title="Upload required documents">
    The creation response includes a `requirements.currentlyDue` array listing every document you need to provide. Each item has a `documentType` — send it in the JSON `body` part of the multipart request, alongside the file:
    ...
    curl --request POST \
      ...
      --header 'Authorization: Bearer <token>' \
      --header 'X-Idempotency-Key: <unique-key>' \
      --form 'body={"documentType":"CNPJ_CERTIFICATE"};type=application/json' \
      --form 'file=@/path/to/cnpj-certificate.pdf'
    ```

    Returns `204 No Content`. Upload one document per request, repeating for every `documentType` listed in `currentlyDue` before proceeding.
  </Step>
  ...
  <Step title="Upload UBO documents">
    Each UBO needs identity documents. Use the same `/documents` endpoint as account documents — just include the UBO's `id` as `uboId` inside the JSON `body`:
    ...
    curl --request POST \
      ...
      --header 'Authorization: Bearer <token>' \
      --header 'X-Idempotency-Key: <unique-key>' \
      --form 'body={"documentType":"IDENTITY_DOCUMENT","uboId":"<ubo-id>"};type=application/json' \
      --form 'file=@/path/to/ubo-identity.pdf'
    ```
