...
If a request arrives with an idempotency key that was already used for a **different** request body, the API returns HTTP `409 Conflict`:
...
If the key matches a previous request with the **same** body, the API returns the original response without reprocessing.
